...
screen buffer and the normal screen buffer using Tea.
...
The `http` example shows how to make an `http` call within your Tea
application.
...
The `mouse` example shows how to receive mouse events in a Tea
application.
...
indicating download progress through Tea.
...
communication with a Tea application.
...
The `textarea` example demonstrates a simple Tea application using a
`textarea` bubble.
...
The `views` example demonstrates how to build a Tea application with
