...
func RequireEqualOutput(tb testing.TB, out []byte) {
	...
	if *update {
		assert.NoError(tb, os.MkdirAll(filepath.Dir(golden), 0o755))
		assert.NoError(tb, os.WriteFile(golden, out, 0o600))
	}

	path := filepath.Join(tb.TempDir(), tb.Name()+".out")
	assert.NoError(tb, os.MkdirAll(filepath.Dir(path), 0o755))
	assert.NoError(tb, os.WriteFile(path, out, 0o600))
